...
function mouse() {
    ...
    var card = document.querySelectorAll(".dragcard");
    ...
    card.forEach(function (elem) {
        ...
        // main.addEventListener("mousemove",function(dets1){
        //    gsap.to(mouse,{
        //     x:dets1.x+"px",
        //     y:dets1.y+"px",
        //     scale:0
...
var loaderCount = document.querySelector(".circleload > h1");
var grow = 0;
var lt = setInterval(function () {
    if (grow < 100) {
        grow += Math.floor(Math.random() * 10) + 1;
        grow = Math.min(grow, 100);
        loaderCount.innerHTML = grow + "%";
    } else {
        clearInterval(lt);
        gsap.to(load, {
            top: "-100vh"
